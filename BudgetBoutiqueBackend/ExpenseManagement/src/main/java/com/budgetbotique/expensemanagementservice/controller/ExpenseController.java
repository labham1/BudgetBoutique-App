...
public class ExpenseController {
 ...
 @GetMapping("/user/getFilterExpense")
 public ResponseEntity<List<Expense>> filterExpenses(
     ...
     try {
         ...
         logger.info("Filtering expenses: category={}, subcategory={}, year={}, month={}, week={}",
                     category, subcategory, year, month, week);

         // Assuming you have a service method to handle the filtering
         List<Expense> filteredExpenses = expenseService.filterExpenses(category, subcategory, year, month, week);
         ...
         filteredExpenses.forEach(expense -> logger.info("Filtered Expense: {}", expense));

         return ResponseEntity.ok(filteredExpenses);
     } catch (Exception e) {
         logger.error("Error filtering expenses for email: {}", email, e);
         return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
     }
 }
